...
public class Database extends SQLiteOpenHelper {
    ...
    public String getBest2048(Context context)
    {
        Cursor cursor = new Database(context).getData("SELECT *FROM Game2048");
        cursor.move(1);
        return cursor.getString(5);
    }
    public void setBest2048(int best)
    {
        SQLiteDatabase database = getWritableDatabase();
        database.execSQL("UPDATE Game2048 SET diemcao ='" +best +"' WHERE hang='hang1'");
        database.close();
    }
    public String getScore2048(Context context)
    {
        Cursor cursor = new Database(context).getData("SELECT *FROM Game2048");
        cursor.move(2);
        return cursor.getString(5);
    ...
    public void setScore2048(int score)
    {
        SQLiteDatabase database = getWritableDatabase();
        database.execSQL("UPDATE Game2048 SET diemcao ='" +score +"' WHERE hang='hang2'");
        database.close();
    }
    public void setSaveState(int matrix[][])
    {
        SQLiteDatabase database = getWritableDatabase();
    ...
    public int[][]  getSaveState(Context context){
        int a[][]= new int[5][5];
        Cursor cursor = new Database(context).getData("SELECT *FROM Game2048");
        for(int i=1;i<5;i++)
        {
